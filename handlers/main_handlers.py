...
async def start(message: types.Message):
    ...
    await bot.send_message(
        ...
        text="🤖 Привет! Это бот для решения задач по физике. Пожалуйста, введите свой пароль",
    )
...
async def prc_get_user_info_state(message: types.Message, user: User):
    ...
    if user is None:
        await bot.send_message(
            chat_id=message.chat.id,
            text="🔁 Привет! Это бот для решения задач по физике. Пожалуйста, введите свой пароль.\n"
                 "Пароль можно получить у Сергея Вачиковича.\n",
        )
    elif user.type == USER_TYPE.DELETED:
        await bot.send_message(
            chat_id=message.chat.id,
            text="🔁 Этот пароль был заблокирован.\n"
                 "Скорее всего новый пароль был получен от Сергея Вачиковича. Сверьте с ним ваши данные.",
        )
    else:
